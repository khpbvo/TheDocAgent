...
from .diff_display import DIM, RESET, render_text_diff
...
class DocxEditor(DocumentEditor):
    ...
    def apply_operation(self, operation: DocumentOperation) -> OperationResult:
        ...
        try:
            from docx import Document  # noqa: F401 - check availability
        except ImportError:
        ...
        if operation.type == OperationType.DOCX_INSERT_TEXT:
            return self._insert_text(path, operation)
        if operation.type == OperationType.DOCX_DELETE_TEXT:
            return self._delete_text(path, operation)
        if operation.type == OperationType.DOCX_ADD_COMMENT:
    ...
    def _replace_text(
        self, path: Path, operation: DocumentOperation
    ) -> OperationResult:
        ...
        if replacements == 0:
            return OperationResult(
                success=False,
                output=(
                    f"Text not found: '{old_text[:50]}...' "
                    if len(old_text) > 50
                    else f"Text not found: '{old_text}'"
                ),
            )
    ...
    def _insert_text(self, path: Path, operation: DocumentOperation) -> OperationResult:
        ...
        if (
            isinstance(operation, DocxOperation)
            and operation.paragraph_index is not None
        ):
    ...
    def create_replace_operation(
        ...
        return DocxOperation(
            ...
            path=path,
            description=description or "Replace text",
            old_text=old_text,
            new_text=new_text,
            location=location,
