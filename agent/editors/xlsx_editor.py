...
from .diff_display import CYAN, DIM, RESET, render_cell_diff
...
class XlsxEditor(DocumentEditor):
    ...
    def _render_range_diff(self, operation: DocumentOperation) -> str:
        ...
        lines = []
        location = (
            f"{operation.sheet}!{operation.cell_range}"
            if operation.sheet
            else operation.cell_range
        )
        lines.append(f"{CYAN}Range: {location}{RESET}")

        if isinstance(operation.old_value, list) and isinstance(
            ...
            for i, (old_row, new_row) in enumerate(
                zip(old_rows[:max_preview], new_rows[:max_preview])
            ):
                if old_row != new_row:
    ...
    def apply_operation(self, operation: DocumentOperation) -> OperationResult:
        ...
        try:
            from openpyxl import load_workbook  # noqa: F401 - check availability
        except ImportError:
        ...
        if operation.type == OperationType.XLSX_WRITE_RANGE:
            return self._write_range(path, operation)
        if operation.type == OperationType.XLSX_ADD_FORMULA:
            return self._add_formula(path, operation)
        if operation.type == OperationType.XLSX_DELETE_ROW:
            return self._delete_row(path, operation)
        if operation.type == OperationType.XLSX_INSERT_ROW:
    ...
    def _write_range(self, path: Path, operation: DocumentOperation) -> OperationResult:
        ...
        if sheet_name not in wb.sheetnames:
            return OperationResult(
                success=False, output=f"Sheet not found: {sheet_name}"
            )
