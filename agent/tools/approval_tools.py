            ...
            for i, para in enumerate(doc.paragraphs):
                if old_text in para.text:
                    ...
                    if idx + len(old_text) < len(text):
                        context_after = text[
                            idx + len(old_text) : idx + len(old_text) + 50
                        ].strip()
                    break
    except Exception:
...
def insert_docx_text(
    ...
    from ..editors.docx_editor import DocxOperation
    ...
    location = (
        ...
        else f"After paragraph {paragraph_index}"
    )
...
def update_xlsx_range(
    ...
    rows = len(values)
    cols = (
        max(len(row) if isinstance(row, list) else 1 for row in values) if values else 1
    )
